...
function buildWorkflow(minRisk: string, noFail: boolean, withDiff: boolean): string {
  const scanLine = noFail
    ? `prescripts scan --output-dir . --min-risk ${minRisk} || true`
    : `prescripts scan --output-dir . --min-risk ${minRisk}`;
...
` : "";
  ...
  return `# Generated by prescripts init-ci
...
export async function runInitCi(
  ...
  const workflowPath = join(workflowDir, "prescripts.yml");
  ...
  process.stdout.write(
    `${cc(GREEN, "✓")} ${cc(BOLD, workflowPath)}\n\n` +
      ...
      `  git commit -m "Add prescripts CI workflow"\n`
  );
...
function buildHookScript(minRisk: string): string {
  return `#!/bin/sh
# prescripts pre-commit hook
# Scans the lockfile when package-lock.json or npm-shrinkwrap.json is staged.
# Generated by: prescripts init-hooks
#
# To bypass in an emergency: git commit --no-verify
# To reconfigure: prescripts init-hooks --min-risk <level> --force
...
echo "prescripts: lockfile changed — scanning dependencies..."

# Resolve prescripts binary — never use npx (downloads from registry without scanning)
if command -v prescripts >/dev/null 2>&1; then
  PRESCRIPTS_CMD="prescripts"
elif [ -x "./node_modules/.bin/prescripts" ]; then
  PRESCRIPTS_CMD="./node_modules/.bin/prescripts"
else
  echo ""
  echo "prescripts: binary not found. Cannot scan dependencies before committing."
  echo "  Install globally:     npm install -g prescripts"
  echo "  Or as devDependency:  npm install --save-dev prescripts"
...
if [ $STATUS -ne 0 ]; then
  echo ""
  echo "prescripts: findings at risk >= ${minRisk} detected in staged lockfile."
...
export function buildProgram(): Command {
  ...
  const sharedOptions = (cmd: Command): Command =>
    cmd
      ...
      .option("--api-url <url>", "Hosted prescripts-api URL (optional)")
